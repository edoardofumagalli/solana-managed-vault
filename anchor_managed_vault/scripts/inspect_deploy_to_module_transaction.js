...
function loadFixtureDeployRequest(args) {
  if (!args.fixture) {
    return {};
  }
  ...
  if (!moduleFixture.requests || !moduleFixture.requests.deploy) {
    throw usageError(
      `Fixture module '${args.fixtureModule}' does not include requests.deploy`
    );
  }

  return moduleFixture.requests.deploy;
}
...
function resolveRemainingAccounts(args, fixtureRequest) {
  ...
  if (fixtureRequest.remainingAccounts) {
    return normalizeRemainingAccounts(
      "fixture requests.deploy.remainingAccounts",
...
function resolveRequestBody(args) {
  const fixtureRequest = loadFixtureDeployRequest(args);
  const requestBody = {
...
async function main() {
  ...
  const endpoint = new URL("/transactions/modules/deploy", args.backendUrl);
...
main().catch((error) => {
  console.error("Deploy to module transaction inspection failed:");
