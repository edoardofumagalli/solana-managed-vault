...
function loadFixtureSyncNavRequest(args) {
  if (!args.fixture) {
    return {};
  }
  ...
  if (!moduleFixture.requests || !moduleFixture.requests.syncNav) {
    throw usageError(
      `Fixture module '${args.fixtureModule}' does not include requests.syncNav`
    );
  }

  return moduleFixture.requests.syncNav;
}

function resolveRequestBody(args) {
  const fixtureRequest = loadFixtureSyncNavRequest(args);
  const requestBody = {
  ...
  const requiredFields = ["vault", "moduleEntry", "feePayer"];
...
async function main() {
  ...
  const endpoint = new URL("/transactions/modules/sync-nav", args.backendUrl);
...
main().catch((error) => {
  console.error("Sync module NAV transaction inspection failed:");
