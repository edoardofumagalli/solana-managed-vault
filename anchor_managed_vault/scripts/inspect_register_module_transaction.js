...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (key === "backend-url") {
      ...
      args.manager = value;
    } else if (key === "module-program") {
      args.moduleProgram = value;
    } else if (key === "module-state") {
      args.moduleState = value;
    } else if (key === "module-underlying-token-account") {
      args.moduleUnderlyingTokenAccount = value;
    } else if (key === "policy-seed") {
      args.policySeed = value;
    } else if (key === "output") {
      args.output = value;
    } else {
...
function loadFixtureRegisterRequest(args) {
  if (!args.fixture) {
    return {};
  }
  ...
  if (!moduleFixture.requests || !moduleFixture.requests.register) {
    throw usageError(
      `Fixture module '${args.fixtureModule}' does not include requests.register`
    );
  }

  return moduleFixture.requests.register;
}

function resolveRequestBody(args) {
  const fixtureRequest = loadFixtureRegisterRequest(args);
  const requestBody = {
...
async function main() {
  ...
  const endpoint = new URL("/transactions/modules/register", args.backendUrl);
...
main().catch((error) => {
  console.error("Register module transaction inspection failed:");
