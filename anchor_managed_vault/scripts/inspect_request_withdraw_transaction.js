...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (key === "backend-url") {
      ...
      args.user = value;
    } else if (key === "shares-amount") {
      args.sharesAmount = value;
    } else if (key === "output") {
      args.output = value;
    } else {
  ...
  for (const requiredKey of ["vault", "user", "sharesAmount"]) {
    if (!args[requiredKey]) {
      const cliKey =
        requiredKey === "sharesAmount" ? "shares-amount" : requiredKey;
      throw usageError(`Missing required argument: --${cliKey}`);
    }
  }
...
async function main() {
  ...
  const endpoint = new URL("/transactions/request-withdraw", args.backendUrl);
  const requestBody = {
    vault: args.vault,
    user: args.user,
