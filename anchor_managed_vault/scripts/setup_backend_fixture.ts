...
function parseBoundedInteger(
    ...
    if (
        ...
        throw usageError(`${field} must be an integer between ${min} and ${max}`);
    }
...
function printPlan(
    ...
    console.log(`emergency admin: ${(args.emergencyAdmin ?? manager).toBase58()}`);
    console.log(`decimals: ${args.decimals}`);
    console.log(`max float bps: ${args.maxFloatBps}`);
    console.log(`manager withdraw delay slots: ${args.managerWithdrawDelaySlots}`);
...
async function main(): Promise<void> {
    ...
    console.log(`user share token account: ${fixture.accounts.userShareTokenAccount}`);
