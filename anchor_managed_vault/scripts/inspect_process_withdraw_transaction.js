...
function parseArgs(argv) {
  ...
  for (const requiredKey of ["vault", "user", "ticketIndex", "feePayer"]) {
    if (!args[requiredKey]) {
      const cliKey =
        requiredKey === "ticketIndex"
          ? "ticket-index"
          : requiredKey === "feePayer"
          ? "fee-payer"
          : requiredKey;
...
async function main() {
  ...
  const endpoint = new URL("/transactions/process-withdraw", args.backendUrl);
  const requestBody = {
    vault: args.vault,
    user: args.user,
